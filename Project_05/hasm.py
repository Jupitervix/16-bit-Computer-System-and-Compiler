...
def Pass1(sourceFile):
    ...
    while parser.Advance():
        ...
        if commandType == NO_COMMAND:
            ...
            value = address
            symbol = parser.Line().strip('(').strip(')') 
            if symbolTable.Contains(symbol) == True:
                Error('Label already exists ' + symbol, value, parser.Line())
            else:
                output = symbolTable.AddEntry(symbol, value)
                if output == False:
                    Error('Invalid label name ' + symbol, value, parser.Line())
